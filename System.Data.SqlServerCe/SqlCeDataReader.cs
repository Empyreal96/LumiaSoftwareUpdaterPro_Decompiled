...
namespace System.Data.SqlServerCe
{
  public class SqlCeDataReader : DbDataReader
  {
    ...
    private static bool IsAliased(MetaData info) => info.ColumnName != null && info.baseColumnName != null && !info.ColumnName.Equals(info.baseColumnName);

    protected bool IsCommandBehavior(CommandBehavior condition) => condition == (condition & this.behavior);
    ...
    internal unsafe void FillMetaData(SqlCeCommand command)
    {
      ...
      finally
      {
        for (int index = 0; index < medbkeyinfoArray.Length; ++index)
          Marshal.FreeCoTaskMem(medbkeyinfoArray[index].pwszBaseColumnName);
      }
    }
      ...
      bool allowMultipleAccessor)
    {
      ...
      for (int index1 = 0; index1 < columnCount; ++index1)
      {
        ...
        if (metaData.typeMap.isBLOB)
        {
          ...
          IntPtr ptr = Marshal.AllocCoTaskMem(ADP.SizeOf_tagDBOBJECT);
          if (IntPtr.Zero == ptr)
    ...
    public virtual SqlBoolean GetSqlBoolean(int ordinal) => this.IsDBNull(ordinal) ? SqlBoolean.Null : new SqlBoolean(this.GetBoolean(ordinal));

    public virtual SqlMoney GetSqlMoney(int ordinal) => this.IsDBNull(ordinal) ? SqlMoney.Null : new SqlMoney(this.GetDecimal(ordinal));
    ...
    public virtual SqlByte GetSqlByte(int ordinal) => this.IsDBNull(ordinal) ? SqlByte.Null : new SqlByte(this.GetByte(ordinal));
    ...
    public virtual SqlDateTime GetSqlDateTime(int ordinal) => this.IsDBNull(ordinal) ? SqlDateTime.Null : new SqlDateTime(this.GetDateTime(ordinal));

    public virtual SqlDouble GetSqlDouble(int ordinal) => this.IsDBNull(ordinal) ? SqlDouble.Null : new SqlDouble(this.GetDouble(ordinal));

    public virtual SqlSingle GetSqlSingle(int ordinal) => this.IsDBNull(ordinal) ? SqlSingle.Null : new SqlSingle(this.GetFloat(ordinal));

    public virtual SqlGuid GetSqlGuid(int ordinal) => this.IsDBNull(ordinal) ? SqlGuid.Null : new SqlGuid(this.GetGuid(ordinal));

    public virtual SqlInt16 GetSqlInt16(int ordinal) => this.IsDBNull(ordinal) ? SqlInt16.Null : new SqlInt16(this.GetInt16(ordinal));

    public virtual SqlInt32 GetSqlInt32(int ordinal) => this.IsDBNull(ordinal) ? SqlInt32.Null : new SqlInt32(this.GetInt32(ordinal));

    public virtual SqlInt64 GetSqlInt64(int ordinal) => this.IsDBNull(ordinal) ? SqlInt64.Null : new SqlInt64(this.GetInt64(ordinal));

    public virtual SqlString GetSqlString(int ordinal) => this.IsDBNull(ordinal) ? SqlString.Null : new SqlString(this.GetString(ordinal));
